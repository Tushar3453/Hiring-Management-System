  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        // Fallback Dummy Data 
        setFeaturedJobs([
          { id: '1', title: 'Frontend Developer', companyName: 'TechCorp', location: 'Bangalore', salary: '₹12 LPA', jobType: 'Full-time' },
          { id: '2', title: 'Product Designer', companyName: 'CreativeStudio', location: 'Mumbai', salary: '₹8 LPA', jobType: 'Remote' },
          { id: '3', title: 'Backend Engineer', companyName: 'DataSystems', location: 'Delhi NCR', salary: '₹15 LPA', jobType: 'Full-time' },
        ]);
      } finally {
        setLoading(false);
      }
      ...
      <div className="relative bg-gradient-to-b from-blue-50 to-white pt-20 pb-24 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="bg-white p-3 rounded-2xl shadow-xl border border-gray-100 flex flex-col sm:flex-row gap-2 max-w-3xl mx-auto">
            ...
            <button className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-8 rounded-xl transition-all shadow-md hover:shadow-lg transform active:scale-95">
              Search
            </button>
          </div>

          <div className="mt-8 text-sm text-gray-500 font-medium">
            Popular: <span className="text-gray-700 hover:text-blue-600 cursor-pointer mx-1">Frontend</span> • <span className="text-gray-700 hover:text-blue-600 cursor-pointer mx-1">Backend</span> • <span className="text-gray-700 hover:text-blue-600 cursor-pointer mx-1">Design</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {loading ? (
            ...
            featuredJobs.map((job) => (
              <div key={job.id} className="group bg-white rounded-2xl border border-gray-100 p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                  ...
                  <Link to="/login" className="text-sm font-medium text-blue-600 hover:underline">Apply Now</Link>
                </div>
              </div>
            ))
          )}
